...
async function run() {
  ...
  assert.deepStrictEqual(summarizeSyncItems([
    ...
    { status: 'SYNCED' },
    { status: 'FAILED' }
  ]), { pending: 1, synced: 2, failed: 1, skipped: 0, stale: 0, deleted: 0, delete_failed: 0, total: 4 });
  ...
  const staleUpdates = [];
  const staleConnection = await ensureDefaultKnowledgeBaseConnection({
    prisma: {
      aiKnowledgeBaseConnection: {
        findFirst: async () => ({ id: 99, tenantCode: 'default', isDefault: true, remoteId: 'missing-remote', collectionName: 'missing_collection' }),
        update: async (payload) => {
          staleUpdates.push(payload);
          return { id: payload.where.id, ...payload.data };
        },
        create: async () => { throw new Error('should update stale default connection, not create a duplicate'); }
      }
    },
    eurobotClient: {
      getDefaultKnowledgeBaseName: () => 'training-tenant-a',
      listInternalCollections: async () => ({ items: [{ id: 'remote-new', name: 'training-tenant-a', collection_name: 'training_tenant_a_new' }] }),
      createInternalCollection: async () => { throw new Error('should reuse matching remote collection'); }
    }
  });
  assert.strictEqual(staleConnection.remoteId, 'remote-new');
  assert.strictEqual(staleConnection.collectionName, 'training_tenant_a_new');
  assert.strictEqual(staleUpdates[0].where.id, 99);
  ...
  let deletedItem = {
    ...
    sourceHash: 'old-hash',
    status: 'SYNCED',
    remoteFileId: 'remote-old-doc'
  };
  const deleteCalls = [];
  const deleteResult = await refreshKnowledgeBase({
    prisma: {
      aiKnowledgeBaseConnection: {
        findFirst: async () => ({ id: 1, remoteId: 'remote-1', collectionName: 'training_tenant_a', status: 'ACTIVE' }),
        update: async ({ data }) => ({ id: 1, remoteId: 'remote-1', collectionName: 'training_tenant_a', status: 'ACTIVE', ...data })
      ...
      aiKnowledgeBaseSyncItem: {
        ...
        upsert: async ({ create }) => ({ id: 7, ...create }),
        update: async ({ where, data }) => {
          if (where.id === deletedItem.id) {
            deletedItem = { ...deletedItem, ...data };
            return deletedItem;
          }
          return { id: where.id, ...data };
        },
        findMany: async () => [deletedItem]
      }
    },
    eurobotClient: {
      getDefaultKnowledgeBaseName: () => 'training-tenant-a',
      listInternalCollections: async () => ({ items: [{ id: 'remote-1', name: 'training-tenant-a', collection_name: 'training_tenant_a' }] }),
      uploadFilesToInternalCollection: async () => ({ count: 1, results: [{ done: true, pipeline: { doc_id: 'doc-current' } }] }),
      deleteFileFromInternalCollection: async (payload) => deleteCalls.push(payload)
    }
  });
  assert.deepStrictEqual(deleteCalls, [{ collectionId: 'remote-1', fileId: 'remote-old-doc' }]);
  assert.strictEqual(deleteResult.summary.deleted, 1);
